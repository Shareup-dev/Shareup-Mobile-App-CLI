...
class UserService {
  ...
  editProfile = (email, data) =>
    AuthAxios.put(`users/${email}/edit_profile`, data);
  ...
  searchFriends = (uid, keyword) =>
    AuthAxios.get(`/friends/search/${uid}?keyword=${keyword}`);
  getFriends = email => AuthAxios.get(`friends/email/${email}`);
  sendFriendRequest = (uid, fid) =>
    AuthAxios.post(`${uid}/friend_request/${fid}`);
  acceptFriendRequest = (uid, fid) =>
    AuthAxios.post(`${uid}/accept_friend_request/${fid}`);
  declineFriendRequest = (uid, fid) =>
